...
export class AppSyncCdkStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    const graphqlApi = new GraphqlApi(
      this,
      `${process.env.SERVICES_PREFIX}_Api`,
      {
        name: `${process.env.SERVICES_PREFIX}_Api`,
        schema: Schema.fromAsset(
          path.join(__dirname, process.env.APPSYNC_SCHEMA_PATH || ``)
        ),
        authorizationConfig: {
    ...
    const lambdaExecutionRole = new Role(
      this,
      `${process.env.SERVICES_PREFIX}_FuncExecutionRole`,
      {
    ...
    const lambdaFunction = new NodejsFunction(
      this,
      `${process.env.SERVICES_PREFIX}_Function`,
      {
    ...
    const dataSource_none = new NoneDataSource(
      this,
